...
void CGameStateInit::OnInit()
{
	...
	aboutText.LoadBitmap(IDB_ABOUTTEXT, RGB(255, 255, 255));
...
void CGameStateInit::OnShow()
{
	...
	if (!inChooseStage) {
		...
		aboutText.SetTopLeft(620, 450);
		menu.ShowBitmap();
		play.ShowBitmap();
		aboutText.ShowBitmap();
		homepage.ShowBitmap();
...
void CGameStateRun::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	const char KEY_NEXTSTAGE = 0x4E; // N 鍵

	if (nChar == KEY_RESET) {
		OnBeginState();
	}

	if (nChar == KEY_NEXTSTAGE) {
